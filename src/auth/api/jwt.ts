...
export const JwtConfigSchema = z.object({
    ...
    AUTH_JWT_ENCRYPTION_KEY_HEX_ENCODED: z.string().regex(/^[0-9A-Fa-f]{64}$/, "invalid key, should be 256 bit (32 byte) crypto random key encoded as 64 hexadecimal characters")
});
 ...
 */
export const generateServiceJwt = (context: Context<unknown>, event: ServerlessEventObject<unknown>, subject: string, additionalJwtClaims?: {[key:string]: unknown}): string => {
...
interface ServiceJwtAuthenticationHandlerSuccessResult<T extends JwtPayloadStandardProps> extends ServiceJwtAuthenticationHandlerResultBase {
    isAuthenticated: true,
    jwt: string,
    claims: T
}

export type ServiceJwtAuthenticationHandlerResult<T extends JwtPayloadStandardProps> = ServiceJwtAuthenticationHandlerFailureResult | ServiceJwtAuthenticationHandlerSuccessResult<T>;

export type ServiceJwtAuthenticationHandler = 
<AdditionalJwtClaimsSchema extends ZodObject<{}>>(context: Context<unknown>, event: ServerlessEventObject<unknown>, jwtRequestParameter: RequestParameter, additionalJwtClaimsSchema: AdditionalJwtClaimsSchema) 
=> Promise<ServiceJwtAuthenticationHandlerResult<JwtPayloadStandardProps & z.infer<typeof additionalJwtClaimsSchema>>>;
 ...
 */
export const serviceJwtAuthenticationHandler: ServiceJwtAuthenticationHandler = (context, event, jwtRequestParameter, additionalJwtClaimsSchema) => {
    const config = parseJwtConfigFromEnv(context);
    ...
    try{
        ...
        const sessionParseResult = additionalJwtClaimsSchema.safeParse(typeof(decoded) === "string" ? JSON.parse(decoded) : decoded);
        if(sessionParseResult.success){
            const okRes: ServiceJwtAuthenticationHandlerSuccessResult<JwtPayloadStandardProps & z.infer<typeof additionalJwtClaimsSchema>> = {
                isAuthenticated: true,
                authenticationMethod: "ServiceJwt",
                jwt: jwtFromRequest,
